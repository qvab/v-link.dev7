  ...
  <form action="" method="GET">
    <div class="row">
      <div class="form-group col-xs-12 col-sm-4">
        ...
        <input type="text" name="search[key]" id="search-key" placeholder="Ключевые слова, названия специализации"
               class="input-group form-control" autocomplete="off" value="<?=$arListFilter["key"]?>"/>
      ...
      <div class="form-group col-xs-12 col-sm-4">
        ...
        <div class="checkall-group">
          <div class="checkbox">
            <input type="checkbox" id="type_of_emp"
                   name="search[type_of_emp][]" <?=empty($arListFilter["type_of_emp"]) ? "checked" : ""?>>
            <label for="type_of_emp-<?=$idType?>">Любое</label>
          </div>
          <?php foreach ($arTypeOFEmp as $idType => $arType) { ?>
            <div class="checkbox">
              <input type="checkbox" id="type_of_emp-<?=$idType?>" name="search[type_of_emp][]"
                     value="<?=$arType["fields"]["ID"]?>" <?=in_array($idType, $arListFilter["type_of_emp"]) ? "checked" : ""?>>
        ...
        <div class="checkall-group">
          <div class="checkbox">
            <input type="checkbox" id="schedule"
                   name="search[schedule][]" <?=empty($arListFilter["type_of_emp"]) ? "checked" : ""?>>
            <label for="schedule">Любое</label>
          </div>
          <?php foreach ($arSchedule as $idType => $arType) { ?>
            <div class="checkbox">
              <input type="checkbox" id="schedule-<?=$idType?>" name="search[schedule][]"
                     value="<?=$arType["fields"]["ID"]?>" <?=in_array($idType, $arListFilter["schedule"]) ? "checked" : ""?>>
